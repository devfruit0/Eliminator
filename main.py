...
while True:
        ...
        # if trying to detect something else without the motion tracking part
            ...
            if detected_object is not None:
                ...
                cv2.rectangle(frame, (detected_object[0], detected_object[1]),
                              ...
                              (45, 90, 255), 3)
                cv2.putText(frame, 'Detected: ' + DETECTION, (detected_object[0], detected_object[1] - 3), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (45, 90, 255), 2)
                checking_times += 1
    ...
    # Enter Validation mode
        ...
        if success:
            ...
            cv2.putText(frame, 'Detected A: ' + str(angle) + ', Servo A: ' + str(int(SERVO.angle)) + ', T: ' +
            ...
            if abs(angle) < SHOOT_TOLERANCE:
                ...
                checking_times = 0
                # current_angle = 0
            elif abs(angle - previous_detection_angle) >= DETECTION_DIFFERENCE_THRESHOLD:
                adjustAngle(np.clip(angle + SERVO.angle, -45, 45))
                # current_angle = angle + current_angle

            previous_detection_angle = angle
